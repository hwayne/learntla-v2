...
Optimizing models is more an art than a science. It's a skill you have to develop over time. Here's some stuff to start you off.
...
By default, the toolbox uses TK of your computer RAM and one worker thread per CPU core. If you instead run TLA+ from the command line, it will default to using 25% RAM and *exactly one* worker. 

.. rubric:: 3. Check your hardware

Model checking is both CPU- and memory-intensive. At times, it's even *single-core* intensive, as the liveness checking algorithm for TLC is single-threaded.
...
To discuss state space optimizations, we'll use the following:

.. spec

Run with ``MaxNum = 7``, ``Workers = {w1, w2, w3}``, and no deadlock checking. You should get 28,351,303 states. I'll be showing optimizations one-at-a-time as changes to this base model, and undo them before showing the next optimization.
...
.. tip::
  ...
  ..todo:: This reduces the state space by TODO percent, making it easier for me to try out different optimizations. When I'm done I'll remove the constraint and see how much my optimizations help with the whole spec.
...
As a very rough rule of thumb, if you take an n-element set of model values and make it a :ref:`Symmetry sets <symmetry_set>`, you'll decrease the state space by a factor of about ``n!``. In this case, making ``Workers`` a symmetry set reduces the state space to 4.7M states, a reduction of about 6.

Separate safety and liveness
............................

This isn't pertitent to this spec, but you should always have separate models for checking safety properties (invariants and :ref:`action properties <action_properties>`) and liveness (everything else). Liveness checking is a *lot* slower than safety and it prevents symmetry set optimizations. Use smaller constants for your liveness checking.
...
This change reduces the state space to 11 million states, a 3x reduction.
...
Here's a trick to estimate how much overhead this adds: let's assume each worker has exactly one number in its ``to_process``, so each of the three workers does exactly two steps in sequence. While each worker has to do the two steps in order, the workers are independent of each other. Then there will be 
  ...
  .. math:: \frac{\left(2+2+2\right)!}{2\fact! \cdot 2!\cdot 2!)} = 90
...
This is an advanced technique and should only be used with caution.
...
If you're running a small model with a lot of RAM, the JVM can waste a lot of time preallocating memory you won't use. 
